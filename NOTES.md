# timesync: detailed notes and device logs

First-time deployment instructions are in [README.md](README.md). This file is the
long version: why each mode behaves the way it does, what was measured on real hardware,
...
config file. A flag cannot be silently discarded the way a config key can, and unlike a
config key it is visible in `ps`, so what the daemon was actually told is never in doubt.
`verify` checks the mode from three directions: the recorded value, the arguments the
unit passes, and the `/dev/ptp*` descriptors the running `ptp4l` holds open.
...
`--advertise-gnss-quality`. `verify` returns 5 only when there is no GNSS reference at
all; a recipe that is present but missing `--advertise-gnss-quality` is an ordinary failed
check, exit 4.
